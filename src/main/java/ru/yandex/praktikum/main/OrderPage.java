...
public class OrderPage {
    ...
    // Используемые Элементы со страницы заказа
...
// Конструктор класса OrderPage, который принимает и инициализирует WebDriver
        this.driver = driver;
    }

    public void waitForLoadOrderPage() { // Метод для ожидания загрузки страницы с заказом.
    ...
    public void setDataFieldsAndClickNext(String valueName, String valueSurname, String valueAddress,
        ...
        // Установка значений в поля Имя, Фамилия, Адрес и Телефон с использованием метода sendKeys()
        ...
        getButtonNext().click(); // Нажатие на кнопку "Далее"
    }
    ...
    // Метод для установки данных в другие поля и нажатия на кнопку "Заказать"
    public void setOtherFieldsAndClickOrder(String valueDateOrder, String valuePeriod, String valueColor, String valueComment) {
        getCalendar().sendKeys(valueDateOrder, Keys.ARROW_DOWN, Keys.ENTER); // Установка значения в поле Календарь
        getPeriod().click(); // Нажатие на период аренды
        getDays(valuePeriod).click();
        getColor(valueColor).click(); // Выбор цвета
        getComment().sendKeys(valueComment); // Ввод комментария
        getButtonOrder().click(); // Нажатие на кнопку "Заказать"
        new WebDriverWait(driver, Duration.ofSeconds(3)).until(driver -> (getButtonYes().isDisplayed())); // Ожидание появления кнопки "Да"
        getButtonYes().click(); // Нажатие на кнопку "Да"
    }
